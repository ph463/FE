...
namespace FE_Berechnungen.Tragwerksberechnung.Modelldaten
{
    public class BiegebalkenGelenk : AbstraktBalken
    {
        ...
        //private readonly double[] kclxinvkll = new double[5];
        private readonly double[] uc = new double[5];
        //private readonly double[] pc = new double[5];
        //private readonly double[] pl = new double[1];
        //private readonly double[] kclxinvkllxpl = new double[5];
        private readonly double[] kcxuc = new double[5];

        //private readonly double[] p = new double[6];
        ...
        //private readonly double[] loadVector = new double[5];
        //public double[] barEndForceVector = new double[6];

        private readonly int[] c;
        private readonly int[] l;

        public BiegebalkenGelenk(string[] eNodeIds, string eMaterialId, string eCrossSectionId, FEModell feModel, int type)
        {
            modell = feModel;
            ElementDof = 3;
            NodeIds = eNodeIds;
        ...
        private double[,] ComputeLocalMatrix()
        {
            ...
            var c1 = ElementMaterial.MaterialWerte[0] * ElementCrossSection.QuerschnittsWerte[0] / length; // AE/L
        ...
        public override double[] ComputeElementState()
        {
            ...
            ElementDeformations = ComputeZustandsvektor();

            // Beitrag der Knotenverformungen
            MatrizenAlgebra.Mult(kcxuc, matrix, ElementDeformations);
        ...
        // ... compute Verformungsvektor für Rahmenelemente .............
        public int[] GetSystemIndices()
        {
            ...
            if (Type != second) throw new ModellAusnahme("BiegebalkenGelenk GetSystemIndices: ungültiger Gelenktyp");
            {
                var reduced = new int[5];
                indices = Nodes[0].SystemIndices;
                reduced[0] = indices[0];
        ...
        private double[,] CondensateMatrix(double[,] ke)
        {
            ...
            for (var i = 0; i < 5; i++)
                for (var j = 0; j < 5; j++) redStiffnessMatrix[i, j] = kcc[i, j] - kclxkllxklc[i, j];
