...
namespace FEALibrary.EquationSolver
{
    ...
    class SymmetricSolverStatus
    {
        ...
        private double GetValue(int i, int m) { return matrix[i][m]; }

        private void SetValue(int i, int m, double value) { matrix[i][m] = value; }
        ...
        public void Decompose()
        {
            ...
            for (row = 0; row < dimension; row++)
            {
                ...
                if (sum < Epsilon) throw new AlgebraicException("SymmetricSolverStatus: Element <= 0 bei Dreieckszerlegung in Zeile " + row);
