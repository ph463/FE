    ...
    class SymmetricSolver
    {
        ...
        {    //triangularization of coefficient matrix A = U * U^T
            ...
            for (s = 0; s < dimension; s++)
            {
                ...
                if (Math.Abs(sum) < Epsilon) throw new AlgebraicException("SymmetricSolver: Pivot in Zeile " + s + " kleiner als Fehlerschranke");
                if (sum < 0) throw new AlgebraicException("SymmetricSolver: negtive Wurzel eines Elementes auf der Hauptdiagonale");
