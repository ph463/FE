namespace FEALibrary.Model
{
    public class FeParser
    {
        ...
        public FeModel FeModell { get; set; }
        ...
        public void ParseNodes(string[] lines)
        {
            for (var i = 0; i < lines.Length; i++)
            {
                ...
                if (lines[i] == "Knotengruppe")
                {
                    ...
                    {
                        throw new ParseException((i + 2) + ": Knotengruppe");
                    }
                    zaehler = 0;
                    i += 2;
                    do
                    {
                        ...
                        {
                            throw new ParseException(i + ": Knotengruppe");
                        }
                ...
                if (lines[i] == "Aequidistantes Knotennetz")
                {
                    do
                    {
                        ...
                        switch (substrings.Length)
                        {
                            ...
                            default:
                                {
                                    throw new ParseException((i + 3) + ": Aequidistantes Knotennetz");
                                }
                        }
                        i++;
                    } while (lines[i + 1].Length != 0);
                ...
                if (lines[i] != "Variables Knotennetz") continue;
                {
                    do
                    {
                        ...
                        switch (substrings.Length)
                        {
                            ...
                            default:
                                {
                                    throw new ParseException((i + 3) + ": Variables Knotennetz");
                                }
                        }
